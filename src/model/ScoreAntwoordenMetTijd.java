...
public class ScoreAntwoordenMetTijd implements QuizScore {
	...
	//maximum aantal punten bij goed antwoord binnen tijd, 0 bij antwoord dat fout is of buiten tijd
	@Override
	public int getScore() {
		double maxScore = 0.0;
		double somScore = 0.0;
		...
		for(OpdrachtAntwoord oa : quizDeelname.getOpdrachtAntwoorden()){
			...
			if(oa.getQuizopdracht().getOpdracht().isJuisteAntwoord(oa.getLaatsteAntwoord()) // antwoord moet juist zijn
					...
					oa.getQuizopdracht().getOpdracht().getMaxAntwoordTijd() == 0)){ // ofwel is er geen max antwoordtijd
				somScore += maxScore;
			}
		}
